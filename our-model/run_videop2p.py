...
def main(
    ...
    class LocalBlend:
        ...
        def __call__(self, x_t, attention_store, step):
            ...
            if self.counter > self.start_blend:
                ...
                maps = [item.reshape(self.alpha_layers.shape[0], -1, 8, 16, 53, MAX_NUM_WORDS) for item in maps]
    ...
    def load_512_seq(image_path, left=0, right=0, top=0, bottom=0, n_sample_frame=video_len, sampling_rate=1):
        ...
        for index in range(n_sample_frame):
            ...
            image = np.array(Image.fromarray(image).resize((1696, 512)))
            frames.append(image)
            
            ### for masks
            p_mask = os.path.join(masks_path, masks[index])
            mask = np.array(Image.open(p_mask).convert("RGB"))
            h_mask, w_mask, c_mask = mask.shape
            left_mask = min(left, w_mask-1)
            right_mask = min(right, w_mask - left_mask - 1)
            top_mask = min(top, h_mask - left_mask - 1)
            bottom_mask = min(bottom, h_mask - top_mask - 1)
            mask = mask[top_mask:h_mask-bottom_mask, left_mask:w_mask-right_mask]
            h_mask, w_mask, c_mask = mask.shape
            """
            if h_mask < w_mask:
                offset_mask = (w_mask - h_mask) // 2
                mask = mask[:, offset_mask:offset_mask + h_mask]
            elif w_mask < h_mask:
                offset_mask = (h_mask - w_mask) // 2
                mask = mask[offset_mask:offset_mask + w_mask]
            """
            #mask = np.array(Image.fromarray(mask).resize((512, 512)))
            mask = np.array(Image.fromarray(mask).resize((1696, 512)))
            frames_mask.append(mask)
        return np.stack(frames), np.stack(frames_mask)
    ...
    class NullInversion:
        ...
        def invert(self, image_path: str, prompt: str, offsets=(0,0,0,0), num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False):
            ...
            image_gt, masks = load_512_seq(image_path, *offsets)
            if verbose:
                print("DDIM inversion...")
            image_rec, ddim_latents, latent = self.ddim_inversion(image_gt)
            if verbose:
                print("Null-text optimization...")
            uncond_embeddings = self.null_optimization(ddim_latents, num_inner_steps, early_stop_epsilon)
            return (image_gt, image_rec), ddim_latents[-1], uncond_embeddings, masks, latent
        ...
        def invert_(self, image_path: str, prompt: str, offsets=(0,0,0,0), num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False):
            ...
            if verbose:
                print("Null-text optimization...")
            return (image_gt, image_rec), ddim_latents[-1], None, masks, latent
    ...
    if fast:
        (image_gt, image_enc), x_t, uncond_embeddings, masks, latent = null_inversion.invert_(image_path, prompt, offsets=(0,0,0,0), verbose=True)
    else:
        (image_gt, image_enc), x_t, uncond_embeddings, masks, latent = null_inversion.invert(image_path, prompt, offsets=(0,0,0,0), verbose=True)
